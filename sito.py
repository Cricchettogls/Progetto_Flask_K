...
def get_quiz_questions():
    # Return 30 random questions for an almost infinite quiz
    import random
...
def profile():
    ...
    # Convert UTC times to Rome timezone (UTC+1)
    from datetime import timezone, timedelta
    rome_tz = timezone(timedelta(hours=1))
    for activity in activities:
        # Convert UTC to Rome timezone
    ...
    try:
        ...
        for i in range(3):
            ...
            if day_forecasts:
                ...
                # Also add the simple reference for backwards compatibility
                if i == 0:
...
def scoreboard():
    ...
    # Add last activity for each player
